...
class SiameseTask(ClassificationTask): pass
...
class HybridTask(ClassificationTask):
    ...
    def validation_step(self, batch, batch_idx): 
        ...
        if self.use_seg:
            # Use maximum segmentation score to aid with `none` class
            # **ASSUMES WE ARE PREDICTING NONE (NOT OPACITY) WITH CLASSIFICATION
            # That's why we take 1 minus the max pixel
            pcls[:,-1] = 0.5*pcls[:,-1] + 0.5*(1.0-torch.amax(p[0], dim=(1,2,3)))
