...
class HybridClsSegLoss(nn.Module):
    ...
    def __init__(self, seg_weight=0.2, pos_frac=0.25, use_weighted_bce=True):
        super().__init__()
...
class MixHybridClsSegLoss(nn.Module):
    ...
    def __init__(self, seg_weight=0.2, use_weighted_bce=False):
        super().__init__()
        self.seg_weight = seg_weight
    ...
    def forward_train(self, p, t):
        lam = t['lam']
        pseg, pcls = p
        cls_loss1 = F.binary_cross_entropy_with_logits(pcls.float(), t['y1_cls'].float(), reduction='none')
        cls_loss2 = F.binary_cross_entropy_with_logits(pcls.float(), t['y2_cls'].float(), reduction='none')
        cls_loss = lam*cls_loss1 + (1-lam)*cls_loss2
        seg_loss1 = F.binary_cross_entropy_with_logits(pseg.float(), t['y1_seg'].float(), reduction='none')
        seg_loss2 = F.binary_cross_entropy_with_logits(pseg.float(), t['y2_seg'].float(), reduction='none')
