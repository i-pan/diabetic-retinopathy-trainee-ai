...
class SegmentationHead(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, dropout=0.0, kernel_size=3, activation=None, upsampling=1):
...
class SegmentationHead3d(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, dropout=0.0, kernel_size=3, activation=None, upsampling=1):
        ...
        upsampling = nn.Upsample(scale_factor=(2,upsampling,upsampling)) if upsampling > 1 else nn.Identity()
        activation = Activation(activation)
        super().__init__(dropout, conv2d, upsampling, activation)
